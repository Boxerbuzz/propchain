...
COMMENT ON FUNCTION get_investments_for_distribution IS 
'Selects investment IDs for distribution with row-level locking (FOR UPDATE SKIP LOCKED) to prevent race conditions. Returns empty array if all investments are locked by other processes.';

COMMENT ON COLUMN tokenizations.last_distribution_at IS 
'Timestamp of last token distribution run. Used for cooldown period enforcement to prevent concurrent distribution attempts.';
