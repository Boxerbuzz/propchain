  ...
  try {
    ...
    if (lockError) {
      ...
      if (existingLock && new Date(existingLock.expires_at) > new Date()) {
        ...
        return new Response(
          JSON.stringify({ error: 'Distribution already in progress', locked_by: existingLock.locked_by }),
          { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      
      // Lock expired, delete and retry
      await supabase.from('token_distribution_locks').delete().eq('tokenization_id', tokenization_id);
    ...
    try {
      ...
      const availableSupply = treasuryTokenBalance ? BigInt(treasuryTokenBalance.toString()) : 0n;
      console.log(`[DISTRIBUTE-TOKENS] Treasury has ${availableSupply} tokens available`);
      ...
      if (target_user_ids && Array.isArray(target_user_ids) && target_user_ids.length > 0) {
        ...
        console.log(`[DISTRIBUTE-TOKENS] Targeting specific users: ${target_user_ids.join(', ')}`);
      }

      const { data: investments, error: investmentError } = await investmentsQuery;
      ...
      console.log(`[DISTRIBUTE-TOKENS] Processing ${userAggregates.size} unique users`);
      ...
      for (const agg of userAggregates.values()) {
        if (agg.kyc_verified && agg.hedera_account_id) {
          ...
          try {
            ...
            const onChainBalance = currentBalance ? BigInt(currentBalance.toString()) : 0n;
            const delta = agg.total_tokens_requested > onChainBalance ? agg.total_tokens_requested - onChainBalance : 0n;
            totalTokensNeeded += delta;
          } catch (e) {
            console.warn(`[DISTRIBUTE-TOKENS] Could not query balance for ${agg.hedera_account_id}, assuming full transfer needed`);
            totalTokensNeeded += agg.total_tokens_requested;
          }
        }
      }

      console.log(`[DISTRIBUTE-TOKENS] Total tokens needed: ${totalTokensNeeded}`);
      
      if (totalTokensNeeded > availableSupply) {
        const shortfall = totalTokensNeeded - availableSupply;
        console.error(`[DISTRIBUTE-TOKENS] INSUFFICIENT TREASURY SUPPLY: Need ${totalTokensNeeded}, have ${availableSupply}, shortfall: ${shortfall}`);
        throw new Error(`Insufficient token supply in treasury. Need ${totalTokensNeeded} tokens but only ${availableSupply} available. Shortfall: ${shortfall}`);
      }

      // Process each user
      for (const [userId, agg] of userAggregates.entries()) {
        console.log(`[DISTRIBUTE-TOKENS] Processing user: ${userId} with ${agg.investment_ids.length} investments`);

        // Check KYC
        if (!agg.kyc_verified) {
          console.log(`[DISTRIBUTE-TOKENS] Skipping user ${userId} - KYC not verified`);
        ...
        if (!agg.hedera_account_id) {
          console.log(`[DISTRIBUTE-TOKENS] Skipping user ${userId} - No Hedera account`);
        ...
        if (walletError || !wallet || !wallet.vault_secret_id) {
          console.log(`[DISTRIBUTE-TOKENS] Skipping user ${userId} - No custodial wallet`);
        ...
        if (vaultError || !userPrivateKey) {
          console.error(`[DISTRIBUTE-TOKENS] Failed to retrieve private key for user ${userId}:`, vaultError);
        ...
        try {
          ...
          console.log(`[DISTRIBUTE-TOKENS] Checking on-chain balance for ${wallet.hedera_account_id}`);
          ...
          try {
            ...
            console.warn('[DISTRIBUTE-TOKENS] Balance query failed:', balanceError.message);
          }

          // Calculate tokens to transfer (idempotent: only send the delta)
          const tokensToTransfer = agg.total_tokens_requested > currentBalance 
            ? agg.total_tokens_requested - currentBalance 
            : 0n;

          if (tokensToTransfer === 0n) {
            console.log(`[DISTRIBUTE-TOKENS] User ${userId} already has ${currentBalance} tokens (expected ${agg.total_tokens_requested}). No transfer needed.`);
            transferTxId = 'no-transfer-needed';
          } else {
            console.log(`[DISTRIBUTE-TOKENS] Will transfer ${tokensToTransfer} tokens to reach target of ${agg.total_tokens_requested}`);

            // Associate token if not already
            const isAssociated = currentBalance > 0n || (await (async () => {
              try {
            ...
            if (!isAssociated) {
              console.log(`[DISTRIBUTE-TOKENS] Associating token with account ${wallet.hedera_account_id}`);
              try {
                ...
                console.log(`[DISTRIBUTE-TOKENS] Token associated: ${associationTxId}`);
            ...
            try {
              ...
              if (kycError.status?._code === 222 || kycError.message?.includes('ACCOUNT_KYC_ALREADY_GRANTED')) {
            ...
            const transferTx = new TransferTransaction()
              ...
              .setTransactionMemo(`Distribution: ${tokenization.properties.title}`)
              .freezeWith(client);
            ...
            console.log(`[DISTRIBUTE-TOKENS] Transfer successful: ${transferTxId}`);
          }

          // SUCCESS: Mark all user's confirmed investments as tokens_distributed
          console.log(`[DISTRIBUTE-TOKENS] Marking ${agg.investment_ids.length} investments as tokens_distributed`);
          ...
          console.error(`[DISTRIBUTE-TOKENS] Hedera operation failed for user ${userId}:`, hederaError);
