...
serve(async (req) => {
  ...
  try {
    ...
    if (!investment_id) {
      return new Response(
        JSON.stringify({ error: 'Investment ID required' }),
    ...
    console.log(`[PROCESS-COMPLETION] Processing completion for investment: ${investment_id}`);
    ...
    if (investmentError || !investment) {
      console.error('[PROCESS-COMPLETION] Investment not found:', investmentError);
      return new Response(
    ...
    if (incrementError) {
      console.error('[PROCESS-COMPLETION] Failed to increment raise:', incrementError);
    }
    ...
    if (holdingsError) {
      console.error('[PROCESS-COMPLETION] Failed to update holdings:', holdingsError);
    }
    ...
    if (roomError) {
      console.error('[PROCESS-COMPLETION] Failed to create chat room:', roomError);
    } else {
      ...
      if (!votingError && roomId) {
        console.log('[PROCESS-COMPLETION] Adding user to chat room with voting power:', votingPower);
        ...
        if (participantError) {
          console.error('[PROCESS-COMPLETION] Failed to add user to chat:', participantError);
        }
      }
    }
    ...
    const { error: notificationError } = await supabase
      ...
      .insert({
        ...
        message: `Your investment of ₦${investment.amount_ngn.toLocaleString()} in ${investment.tokenizations.properties.title} has been confirmed. You now own ${investment.tokens_requested} ${investment.tokenizations.token_symbol || 'tokens'}.`,
    ...
    if (notificationError) {
      console.error('[PROCESS-COMPLETION] Failed to create notification:', notificationError);
    }

    console.log('[PROCESS-COMPLETION] Investment completion processing finished');
