...
import { TokenizationFormData, TokenizationFormSchema, Tokenization, ApiResponseSchema } from "../types";
import { z } from "zod";
...
const TokenizationResponseSchema = ApiResponseSchema(Tokenization);
type TokenizationResponse = z.infer<typeof TokenizationResponseSchema>;

const TokenizationsListResponseSchema = ApiResponseSchema(z.array(Tokenization));
type TokenizationsListResponse = z.infer<typeof TokenizationsListResponseSchema>;

export const tokenizationApi = {
  async createTokenizationProposal(createdBy: string, propertyId: string, formData: any): Promise<TokenizationResponse | ApiResponseSchema<z.ZodAny>> {
    try {
  ...
  async getTokenizationDetails(tokenizationId: string): Promise<TokenizationResponse | ApiResponseSchema<z.ZodAny>> {
    try {
  ...
  async listTokenizations(filters?: any): Promise<TokenizationsListResponse | ApiResponseSchema<z.ZodAny>> {
    try {
  ...
  async updateTokenizationStatus(tokenizationId: string, status: Tokenization["status"]): Promise<TokenizationResponse | ApiResponseSchema<z.ZodAny>> {
    try {
  ...
  async mintPropertyTokens(tokenizationId: string, supplyKeyString: string): Promise<TokenizationResponse | ApiResponseSchema<z.ZodAny>> {
    try {
