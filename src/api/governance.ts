...
export const governanceApi = {
  async createProposal(proposerId: string, propertyId: string, formData: any): Promise<GovernanceProposalResponse | GenericApiResponse> {
    try {
      ...
      return { success: true, data: proposal, message: "Governance proposal created successfully." };
    } catch (error: any) {
      if (error instanceof z.ZodError) {
        return { success: false, error: error.errors[0].message, message: "Validation Error" };
      }
      return { success: false, error: error.message, message: "Failed to create governance proposal." };
    }
  },
  ...
  async getProposalDetails(proposalId: string): Promise<GovernanceProposalResponse | GenericApiResponse> {
    try {
      ...
      return { success: true, data: proposal, message: "Governance proposal details retrieved successfully." };
    } catch (error: any) {
      return { success: false, error: error.message, message: "Failed to retrieve governance proposal details." };
    }
  },
  ...
  async listProposals(filters?: any): Promise<GovernanceProposalsListResponse | GenericApiResponse> {
    try {
      ...
      return { success: false, error: error.message, message: "Failed to list governance proposals." };
    }
  },
  ...
  async submitVote(proposalId: string, voterId: string, voteChoice: Vote["voteChoice"], votingPower: number, voteReason?: string): Promise<VoteResponse | GenericApiResponse> {
    try {
      ...
      return { success: false, error: error.message, message: "Failed to submit vote." };
    }
  },
  ...
  async getVotesByProposalId(proposalId: string): Promise<VotesListResponse | GenericApiResponse> {
    try {
      ...
      return { success: false, error: error.message, message: "Failed to retrieve votes." };
    }
  },
};
