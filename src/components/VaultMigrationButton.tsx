...
export const VaultMigrationButton = () => {
  ...
  const handleMigration = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `Migration completed! ${data.migrated_count} wallets migrated to secure Vault storage.`,
          {
            description:
              data.error_count > 0
                ? `${data.error_count} errors occurred during migration.`
